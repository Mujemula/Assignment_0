---
title: "Assignment 0 - Data Cleaning - Instructions"
...
```{r}
data_1<- read.csv("demo_train.csv")
data_2<- read.csv("LU_train.csv")
data_3<- read.csv("token_train.csv")
...
data_3 <- data_3 %>% 
  rename(MOT_MLU=types_MOT) %>% 
  rename(CHI_MLU=types_CHI) 
...
```{r}
compare_df_cols(data_1,data_2,data_3)
```
...
data_2 <- data_2 %>% 
  mutate(SUBJ=str_replace_all(SUBJ, "([.])", ""))
view(data_2)
...
```{r}
data_1 <- data_1 %>% 
  select(SUBJ,VISIT,Diagnosis,Ethnicity,Gender,Age,ADOS,MullenRaw,ExpressiveLangRaw,Socialization)
view(data_1)
...
data_3 <- data_3 %>% 
  select(SUBJ,VISIT,MOT_MLU,CHI_MLU,tokens_CHI,tokens_MOT)
...
```{r}
clean_df_try <- merge(data_1,data_2, by="SUBJ","VISIT", all = TRUE)

clean_df_try <- merge(clean_df_try,data_3,by="SUBJ", all = TRUE)
view(clean_df_try)


df_list_try <- list(data_1,data_2,data_3)
clean_try_2 <- df_list_try %>% 
  reduce(full_join, by="SUBJ","VISIT")
view(clean_try_2)

#why is there 2 VISIT columns? Is it because there is an extra SUBJ AA in data_3? 
#Did you change MOT_MLU and CHI_MLU to integers?
```
